...
var holeyMaterial = new THREE.ShaderMaterial( {
        uniforms: {
           ...
           holeyTorusPos: {value:new THREE.Vector3(6.0, 0.0, 0.3)},
           hMatrix: {value: matht},
           cMatrix: {value: new THREE.Matrix4()}
...
var mySphereMaterial = new THREE.ShaderMaterial( {
        uniforms: {
           lightPosition: {value: new THREE.Vector3(0.0,0.0,-1.0) },
           myColor: { value: new THREE.Vector4(1.0,0.5,0.8,1.0) },
           myfloat: {value: 0.0}
        },
	vertexShader: document.getElementById( 'mySphereVertexShader' ).textContent,
	fragmentShader: document.getElementById( 'mySphereFragmentShader' ).textContent
} );
...
////// DONE:  add the other walls of the skybox
...
// holey-shaded torus
/////////////////////////////////////////////////////////////////////////
var holeyTorusPosWCS= new THREE.Vector3(6.0,0.0,0.3); // HOLEythorus pos in wcs
...
holeyTorus=torus;
scene.add( torus );
var matht = new THREE.Matrix4();
matht.getInverse(holeyTorus.matrix);
...
sphereGeometry = new THREE.SphereGeometry(2, 32, 32);    // radius, segments, segments
var mySphere = new THREE.Mesh(sphereGeometry, mySphereMaterial);
mySphere.position.set(10,10,-25);
//mySphere.position.set(light.position.x, light.position.y, light.position.z);
//scene.add(mySphere);
...
function checkKeyboard() {
  if (keyboard.pressed("W")) {
    ...
    var htp= new THREE.Vector3(6.0,0.0,0.3);
    //var htpn= new THREE.Vector3(-6.0,0.0,-0.3);

    //var mat = new THREE.Matrix4();
  //  mat.getInverse(holeyTorus.matrix);
  //  htp.applyMatrix4(camera.matrix)
  //  htp.applyMatrix4(mat)
  ///  var mat = new THREE.Matrix4();
    //mat.getInverse(holeyTorus.matrix);
  ...
  holeyTorusPosWCS.set(holeyTorus.position.x, holeyTorus.position.y, holeyTorus.position.z);
  holeyTorusPosWCS.applyMatrix4(camera.matrixWorldInverse);
  holeyMaterial.uniforms.holeyTorusPos.value = holeyTorusPosWCS;
  holeyMaterial.uniforms.holeyTorusPos.value.needsUpdate=true;

  var mat = new THREE.Matrix4();
  mat.getInverse(holeyTorus.matrix);


  holeyMaterial.uniforms.hMatrix.value = mat;
  holeyMaterial.uniforms.hMatrix.value.needsUpdate=true;

  holeyMaterial.uniforms.cMatrix.value = camera.matrixWorldInverse;
  holeyMaterial.uniforms.cMatrix.value.needsUpdate=true;
  ...
  mySphereMaterial.uniforms.lightPosition.value = vcsLight;
  mySphereMaterial.uniforms.lightPosition.value.needsUpdate = true;



  //console.log(holeyTorus.position.applyMatrix4(camera.matrixWorldInverse));
}
...
function update() {
  ...
  tick = ((tick+1)%100);
  mySphereMaterial.uniforms.myfloat.value = Math.sin((tick/100)*2*Math.PI);
  mySphereMaterial.uniforms.myfloat.value.needsUpdate = true;
